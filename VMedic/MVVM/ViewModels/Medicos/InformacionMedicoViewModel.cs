...
namespace VMedic.MVVM.ViewModels.Medicos
{
    ...
    public partial class InformacionMedicoViewModel : BaseViewModel
    {
        ...
        private async void MostrarDetallesMedico()
        {
            ...
            MedicoColor = Medico?.COLOR switch
            {
                "Rojo" => Colors.Red.ToHex(),
                "Azul" => Colors.Blue.ToHex(),
                "Amarillo" => Colors.Yellow.ToHex(),
                "Verde" => Colors.Green.ToHex(),
                _ => Colors.Black.ToHex(),
            };
            ...
            if (Medico is not null)
            {
                var place = new CustomPin
                {
                    ...
                    Icon = ImageSource.FromStream(() =>
                    {
                        ...
                        {
                            Debug.WriteLine($"Advertencia: El archivo de imagen no se encontró: {resource}");
                            return null;
                        }
                    }),
