...
public class CustomMapHandler : MapHandler
{
    ...
    private static MKAnnotationView GetViewForAnnotations(MKMapView mapView, IMKAnnotation annotation)
    {
        ...
        if (annotation is CustomAnnotation customAnnotation)
        {
            ...
            if (customAnnotation.ClassID == "1")
            {
                var bounce = CABasicAnimation.FromKeyPath("transform.translation.y");
                bounce.Duration = 0.5;
                bounce.From = NSNumber.FromFloat(0f);
                bounce.To = NSNumber.FromFloat(-10f);
                bounce.TimingFunction = CAMediaTimingFunction.FromName(CAMediaTimingFunction.EaseInEaseOut);
                bounce.AutoReverses = true;
                bounce.RepeatCount = float.MaxValue;

                annotationView.Layer.AddAnimation(bounce, "bounce");
            }
        }
